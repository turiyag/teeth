(function($){
    ...
    $.fn.bubble = function (options) {
        var i, sOut, el, ctx, opt = {
            ...
            x:0,
            y:0,
            radius:$.rbetween(50,120),
            wanderradius: 100,
            color:$.rcolor(),
            callback: function(bubble) {return;}
        };
        $.extend(opt,options);
        $(opt.parent).append('<canvas id="' + opt.id + '" width="' + (opt.radius * 2) + '" height="' + (opt.radius * 2) + '"></canvas>');
        el = $("#" + opt.id)[0];
    ...
    $.fn.hoverOrClick = function (callIn, callOut, callFinal) {
        if (Modernizr.touch){
            $(this).click(function(event) {
                if(!$(this).data("secondTouch")) {
                    callIn(event);
                    $(this).data("secondTouch",true)
                } else {
                    callFinal(event);
                }
            });
        } else {
            $(this).click(callFinal).hover(callIn, callOut);
        }
        return this;
    }
